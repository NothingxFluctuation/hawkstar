...
var Csai = function (_akamai$amp$Plugin) {
  ...
  babelHelpers.createClass(Csai, [{
    ...
    value: function onready() {
      ...
      //this.mediaTransform(); //execute custom code for temporal type
    }
  }, {
    key: 'init',
    value: function init() {
      ...
      for (var i = 0, j = data.tagList.length; i < j; i++) {
        ...
        for (var k = 0, l = result.length; k < l; k++) {
          ...
          if (typeof tag !== 'undefined' && 'DURATION' == tag[0]) {
            ...
            breakId = tag[1].match(/^(2)/g) ? 0 : tag[1].indexOf('0x1') > -1 ? 0 : 0;
            // HACK! breakId = tag[1].match(/^(2)/g) ? 0 : tag[1].indexOf('0x1') > -1 ? 0 : 1;
          }
        }
      }
    ...
    value: function mediaTransform() {
      ...
      if (babelHelpers.typeof(player.mediaTransforms) === 'object') {
...
}((this.akamai.amp.csai = this.akamai.amp.csai || {})));
